# 👋 Ferial Zamoun – Data Analyst & Passionnée d'IA

**📊 Portfolio Data & IA** | **RNCP 37837** | **25 ans d'expérience IT**

*Transformation de données dispersées en **décisions fiables**, indicateurs actionnables et restitutions partageables.*

---

## 🎯 Découvrir Mon Portfolio

👉 **[MON-PORTFOLIO-DATA](https://github.com/ferialzamoun-afk/MON-PORTFOLIO-DATA)** — Hub complet avec gouvernance IA, 15 projets, veille technologique et fiches de compétences

### 🚀 Accès Rapide — Projets Phares

| Projet | Type | Description | Lien |
|--------|------|-------------|------|
| **P15** | 🧭 Observatoire Emploi Data & IA | Projet personnel — cartographie du marché de l'emploi Data & IA, confrontation BMO 2026, audit RNCP, DuckDB/Power BI/Streamlit | [🌐 Application en direct](https://8i4kbcfrhfrty7p8ivapeu.streamlit.app/) · [📄 Fiche Projet](https://github.com/ferialzamoun-afk/MON-PORTFOLIO-DATA/blob/main/projets/P15_observatoire_emploi_data/README.md) |
| **P14** | 🏪 Analyse Retail | Stage Intermarché — disponibilité produit, performance assortiment, pipeline Python/Power BI/Streamlit | [📄 Fiche Projet](https://github.com/ferialzamoun-afk/MON-PORTFOLIO-DATA/blob/main/projets/P14_stage/README.md) |
| **P13** | 🏆 Gouvernance IA | Portfolio & RNCP 37837 — Optimisation de notebook (-68%), 18 contrôles qualité, 26 requêtes tracées | [📄 Fiche Projet](https://github.com/ferialzamoun-afk/MON-PORTFOLIO-DATA/tree/main/projets/P13_portfolio) |
| **P12** | 🔍 Classification ML | Détection de Faux Billets — Régression Logistique (99% de précision), API FastAPI, export Joblib | [📄 Fiche Projet](https://github.com/ferialzamoun-afk/MON-PORTFOLIO-DATA/blob/main/projets/P12_faux_billets/README.md) |
| **P11** | 📈 Clustering & ACP | Étude Marché Export — 2 groupes de pays, ACP à 89.9% de variance, recommandations PESTEL | [📄 Fiche Projet](https://github.com/ferialzamoun-afk/MON-PORTFOLIO-DATA/blob/main/projets/P11_etude_marche/README.md) |
| **P10** | 🌊 Tableau de Bord Power BI | ODD ONU Eau Potable — 150+ pays, 5 sources, schéma en étoile, carte thermique | [📄 Fiche Projet](https://github.com/ferialzamoun-afk/MON-PORTFOLIO-DATA/blob/main/projets/P10_eau_potable/README.md) |
| **P9** | 📊 Application Streamlit | Analyse des Ventes Lapage — Ventes en direct, segmentation clients, indicateurs clés | [🌐 Application](https://p9-lapage-app-9jzz7bhjbzdalqkrzaiz62.streamlit.app/Axe_Analytique_Statistiques) |

---

## 📚 Compétences Clés

- **Analyse Exploratoire des Données (EDA)** : Jupyter, Pandas, NumPy, Matplotlib, Seaborn
- **Apprentissage Automatique (Machine Learning)** : scikit-learn (KMeans, ACP, Régression Logistique, Forêts Aléatoires, KNN), Statsmodels
- **Pipelines de Données** : dbt 1.11.3, Snowflake, SQL
- **Visualisation Décisionnelle** : Power BI, Streamlit, Plotly
- **Qualité & Fiabilisation des Données** : Great Expectations, contrôles qualité, validation de données
- **Gouvernance IA** : Traçabilité des requêtes/prompts, documentation des limites, journal des décisions

---

## 🎓 Certifications & Parcours

- **RNCP 37837** — Data Analyst (validé)
- **OpenClassrooms / GRETA** — Formation certifiante
- **25 ans d'expérience IT** — Développement, AMOA, Recette & Tests, Pilotage de projets

---

## 📞 Contact & Liens

- **Email** : [ferial.zamoun@gmail.com](mailto:ferial.zamoun@gmail.com)
- **LinkedIn** : [férial zamoun](https://www.linkedin.com/in/f%C3%A9rialzamoun/)
- **GitHub** : [ferialzamoun-afk](https://github.com/ferialzamoun-afk)
- **Site Portfolio** : [MON-PORTFOLIO-DATA](https://ferialzamoun-afk.github.io/MON-PORTFOLIO-DATA/)

---

**✨ Bienvenue dans mon univers data !**

*Dernière mise à jour : Septembre 2026*
